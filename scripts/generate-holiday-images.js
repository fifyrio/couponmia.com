...
async function updateHolidayPage(holidaySlug, imageUrl) {
  ...
  try {
    ...
    // Parse existing entries from the current mapping content
    const currentMappingContent = content.substring(mappingStart, mappingEnd);
    const existingEntries = new Map();
    
    // Extract existing entries with proper regex
    const entryMatches = currentMappingContent.matchAll(/'([^']+)':\s*'([^']*)',?/g);
    for (const match of entryMatches) {
      existingEntries.set(match[1], match[2]);
    }
    
    // Add or update the current entry
    existingEntries.set(holidaySlug, imageUrl);
    
    // Rebuild the mapping with proper formatting
    let newMapping = 'const holidayImages: { [key: string]: string } = {\n';
    
    // Add comment lines
    newMapping += '  // Generated by generate-holiday-images.js script\n';
    
    // Sort entries by key for consistent ordering
    const sortedEntries = Array.from(existingEntries.entries()).sort();
    
    for (const [key, value] of sortedEntries) {
      if (value) { // Only include entries with actual URLs
        newMapping += `  '${key}': '${value}',\n`;
      }
    }
    
    // Reconstruct the file
    const updatedContent = beforeMapping + newMapping + afterMapping;
