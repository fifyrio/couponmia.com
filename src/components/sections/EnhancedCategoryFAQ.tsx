...
// Simple markdown renderer for MVP - no external dependencies needed
const renderMarkdownContent = (content: string) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('**') && line.endsWith('**')) {
      ...
      elements.push(
        <h4 key={key++} className="font-semibold text-text-primary mb-3 mt-4 first:mt-0">
          {headerText}
        </h4>
      );
      continue;
    ...
    if (line.startsWith('• ')) {
      ...
      elements.push(
        <div key={key++} className="flex items-start mb-2">
          <span className="text-brand-light mr-2 mt-1 flex-shrink-0">•</span>
          <span className="text-text-secondary">{bulletText}</span>
        </div>
      );
      continue;
    }
...
export default function EnhancedCategoryFAQ({ categoryName, faqs }: CategoryFAQProps) {
  ...
  sectionMap.forEach((items, title) => {
    ...
    // Assign priorities based on section type (lower number = higher priority)
    let priority = 5; // Default for FAQ sections (lowest priority)
              ...
              <div className={`overflow-hidden transition-all duration-300 ${
                ...
                <div className="px-6 py-4 bg-card-bg/30 border-t border-card-border">
                  <div className="text-text-secondary leading-relaxed">
                    {renderMarkdownContent(faq.answer.replace(/\{categoryName\}/g, categoryName))}
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="bg-card-bg/40 rounded-xl p-6 border border-card-border/50">
        {section.items.map((item, index) => (
          <div key={item.id} className={index > 0 ? 'mt-6 pt-6 border-t border-card-border/30' : ''}>
            <div className="text-text-secondary leading-relaxed">
              {renderMarkdownContent(item.answer.replace(/\{categoryName\}/g, categoryName))}
            </div>
          </div>
        ))}
      </div>
